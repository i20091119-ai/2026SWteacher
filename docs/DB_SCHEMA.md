...
| `role` | `주` / `보조` / `토오전` / `토오후` / `토종일` / `일오전` / 공란 |
| `name` | 강사 |
| `h_explain`, `h_support`, `h_research` | 시수 (0.5 단위) |

시수는 화면에서 **합계 한 칸**으로 입력받아 유형에 맞는 컬럼에 담습니다.
해설/지원이 나뉘는 활동(주말어드벤처)은 형태·역할별 표준표(`js/hours.js` 의
`STANDARD_HOURS`)대로 자동으로 쪼개집니다. 조회 시 `hours` 로 합계도 함께 내려갑니다.

| 형태 · 역할 | 합계 | 내역 |
|---|---|---|
| 가족체험 · 학교체험 | 3h | 해설 3h |
| 주말어드벤처 토오전 / 토오후 | 3.5h | 해설 3h + 지원 0.5h |
| 주말어드벤처 토종일 | 7h | 해설 6h + 지원 1h |
| 주말어드벤처 일오전 | 4h | 해설 3h + 지원 1h |
| `memo` | |
