...
| `memo` | |
| `carry` | 1이면 이 활동을 당월 장부·금액에서 빼고 다음 달로 미룬다 |

인덱스: `(date)`, `(name, date)`, `(form, role, date)`

`연구이월` / `지원이월` 은 "지난달에 장부에서 뺀 활동을 이번 달에 보전한다"는 뜻의 유형입니다.
`carry` 플래그와 짝을 이루는 **수당 계산 장치**라서, 자동 이월 추천 기능을 없앤 뒤에도
관리자가 직접 편성할 수 있게 남겨두었습니다.
