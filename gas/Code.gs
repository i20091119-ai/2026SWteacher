...
function exportAll() {
  ensureTabs();
  const clean = function (v) { return String(v == null ? "" : v); };

  const instructors = readAll(getSheet(TABS.instructors, HEADERS.instructors))
    .map(function (r) { return { name: clean(r.name).trim(), order: Number(r.order || 0) }; })
    .filter(function (r) { return r.name; });

  const unavailable = readAll(getSheet(TABS.unavail, HEADERS.unavail))
    .map(function (r) {
      return { name: clean(r.name).trim(), date: toDateStr(r.date), reason: clean(r.reason) };
    })
    .filter(function (r) { return r.name && r.date; });

  const submissions = readAll(getSheet(TABS.submit, HEADERS.submit))
    .map(function (r) {
      return {
        ym: ymOf(r.ym), name: clean(r.name).trim(),
        submitted: isTrue(r.submitted), submittedAt: clean(r.submittedAt),
      };
    })
  ...
  const assignments = readAll(getSheet(TABS.schedule, HEADERS.schedule))
    .map(function (r) {
      return {
        id: clean(r.id), date: toDateStr(r.date), kind: clean(r.kind),
        form: clean(r.form), role: clean(r.role), name: clean(r.name).trim(),
        hExplain: Number(r.hExplain || 0), hSupport: Number(r.hSupport || 0),
        hResearch: Number(r.hResearch || 0), memo: clean(r.memo), carry: isTrue(r.carry),
      };
    })
  ...
  const swaps = readAll(getSheet(TABS.swap, HEADERS.swap))
    .map(function (r) {
      return {
        id: clean(r.id), ym: ymOf(r.ym), assignmentId: clean(r.assignmentId),
        requester: clean(r.requester), target: clean(r.target), status: clean(r.status),
        requestedAt: clean(r.requestedAt), respondedAt: clean(r.respondedAt),
        finalizedAt: clean(r.finalizedAt), note: clean(r.note),
      };
    })
    .filter(function (r) { return r.id && r.ym; });

  const programs = readAll(getSheet(TABS.program, HEADERS.program))
    .map(function (r) {
      return {
        id: clean(r.id), dateStart: toDateStr(r.dateStart), dateEnd: toDateStr(r.dateEnd),
        session: clean(r.session), school: clean(r.school),
        students: Number(r.students || 0), note: clean(r.note),
      };
    })
    .filter(function (r) { return r.dateStart && r.school; });

  const settings = readAll(getSheet(TABS.settings, HEADERS.settings))
    .map(function (r) { return { key: clean(r.key), value: clean(r.value) }; })
